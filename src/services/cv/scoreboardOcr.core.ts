/**
 * The tesseract half of the CV Scoreboard Tracker — DOM-free.
 *
 ...
 *
 * Nothing in this file touches the DOM, so the same code runs in a Node worker
 * against ffmpeg-decoded frames. Where the asset paths live, and how a video
 * element becomes pixels, are the caller's business — see `scoreboardOcr.ts`
 * for the browser's answers.
 */
...
import { grayToBmp } from './encodeImage';
...
/**
 * Where to load the tesseract worker, WASM core and language data from.
 *
 * Explicit rather than derived, because the two hosts answer it differently:
 * the browser needs a path under the app's deploy sub-path (which is why this
 * used to read `import.meta.env.BASE_URL`), and Node needs a filesystem path
 * into `node_modules` and `public/`. Deriving it inside this file is what made
 * it browser-only.
 */
export interface OcrConfig {
  ...
  corePath: string;
  langPath: string;
  /** Browser-only: copy the worker to a blob URL so it stays same-origin. */
...
export async function recognizePlane(
  ...
  // Encoded bytes rather than a canvas: tesseract accepts either, and this way
  // the function has no DOM dependency at all. Measured identical in cost —
  // see the benchmark note in `encodeImage.ts`.
  const { data } = await worker.recognize(grayToBmp(plane) as never);
 ...
 * running scores. `frame` is raw pixels; obtaining them is the caller's job
 * (`captureFrame` in the browser, an ffmpeg pipe in Node).
 *
